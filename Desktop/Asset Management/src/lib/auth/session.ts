...
export async function getSessionUser() {
  ...
  return { user, profile: await getProfileByAuthId(user.id) };
}
...
async function getDemoProfileBySession(session: DemoSession): Promise<UserProfile | null> {
  ...
  if (byEmail.data) {
    return { ...(byEmail.data as UserProfile), role: session.role };
  }
  ...
  if (byRole.data) {
    return { ...(byRole.data as UserProfile), role: session.role };
  }
  ...
  if (fallback.data) {
    return { ...(fallback.data as UserProfile), role: session.role };
  }
