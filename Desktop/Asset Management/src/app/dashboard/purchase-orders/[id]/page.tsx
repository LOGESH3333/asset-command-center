...
import { getPurchaseOrder } from '@/lib/supabase/brd/purchase-orders';
import { updatePurchaseOrderAction } from '@/app/actions/brd/purchase-orders';
...
export default function PurchaseOrderDetailPage() {
  ...
  useEffect(() => {
    getPurchaseOrder(id).then(({ data, error: e }) => {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    if (r.error) setError(r.error);
      ...
      getPurchaseOrder(id).then(({ data }) => setRow(data));
    }
    setActing(false);
  };
