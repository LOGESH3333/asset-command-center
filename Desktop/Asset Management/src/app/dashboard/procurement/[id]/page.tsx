...
import { getProcurement } from '@/lib/supabase/brd/procurement';
import { formatRequestLabel } from '@/lib/supabase/requests';
import { updateProcurementAction } from '@/app/actions/brd/procurement';
...
export default function ProcurementDetailPage() {
  ...
  useEffect(() => {
    getProcurement(id).then(({ data, error: e }) => {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    if (r.error) setError(r.error);
      ...
      getProcurement(id).then(({ data }) => setRow(data));
    }
    setActing(false);
  };
