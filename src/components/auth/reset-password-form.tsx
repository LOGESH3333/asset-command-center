...
export function ResetPasswordForm() {
    ...
    const establishRecoverySession = async () => {
      ...
      if (code) {
        ...
        logAuthEvent('password-reset-update', {
          ...
          response: {
            error: serializeAuthError(exchangeError),
      ...
      if (tokenHash && type === 'recovery') {
        ...
        logAuthEvent('password-reset-update', {
          ...
          method: 'verifyOtp',
          response: {
            error: serializeAuthError(verifyError),
            hasSession: Boolean(data.session),
            userId: data.session?.user?.id ?? null,
          },
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    logAuthEvent('password-reset-update', { step: 'password-update-start' });

    const { data, error: updateError } = await supabase.auth.updateUser({ password });

    logAuthEvent('password-reset-update', {
      step: 'password-update-result',
      response: {
    ...
    return (
      <AuthShell heading="Password updated">
        <div className="space-y-4 text-center">
          ...
          <Link href="/login">
            <Button variant="outline" className="w-full rounded-xl">Go to sign in</Button>
          </Link>
        </div>
      </AuthShell>
    );
