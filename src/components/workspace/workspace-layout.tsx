...
export function WorkspaceDataPanel({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[rgba(139,92,246,0.15)] bg-[rgba(7,7,16,0.55)] shadow-xl shadow-black/20 backdrop-blur-xl">
      {toolbar && (
        <div className="border-b border-violet-500/12 bg-[rgba(10,10,20,0.45)] px-5 py-4">{toolbar}</div>
      )}
      <div className="p-5">{children}</div>
      {footer && (
        <div className="border-t border-violet-500/12 bg-[rgba(10,10,20,0.45)] px-5 py-4">{footer}</div>
      )}
    </div>
  );
}
