  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (stream.error) {
          console.warn('Activity feed:', stream.error);
          setItems([]);
          return;
        }

        const feed: FeedItem[] = [];
        const assetRows = (stream.assetRows ?? []) as ActivityAssetRow[];
        const maintenanceRows = (stream.maintCompletedRows ?? []) as ActivityMaintenanceRow[];
        const vendorRows = (stream.vendorRows ?? []) as ActivityVendorRow[];
        const requestRows = (stream.fulfilledRequestRows ?? []) as ActivityRequestRow[];
        const approvalRows = (stream.approvalRows ?? []) as ActivityApprovalRow[];
        const auditRows = (stream.auditRows ?? []) as ActivityAuditRow[];

        assetRows.forEach((a: ActivityAssetRow) => {
          feed.push({
        ...
        maintenanceRows.forEach((m: ActivityMaintenanceRow) =>
          feed.push({
            id: `maint-${m.id}`,
            type: 'maintenance',
        ...
        vendorRows.forEach((v: ActivityVendorRow, i: number) =>
          feed.push({
            id: `vendor-${i}`,
            type: 'vendor',
        ...
        approvalRows.forEach((a: ActivityApprovalRow) => {
          const justification =
            (Array.isArray(a.asset_requests) ? a.asset_requests[0]?.justification : a.asset_requests?.justification);
          feed.push({
            id: `approval-${a.id}`,
            type: 'request',
        ...
        requestRows.forEach((r: ActivityRequestRow) =>
          feed.push({
            id: `req-${r.id}`,
            type: 'request',
        ...
        auditRows.forEach((a: ActivityAuditRow) => {
