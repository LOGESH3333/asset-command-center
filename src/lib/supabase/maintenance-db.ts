...
export async function fetchMaintenanceLookups(): Promise<{
  ...
  const [assetsRes, vendorResult] = await Promise.all([
  ...
  if (vendorResult.error) warnings.push(`Vendors: ${vendorResult.error}`);

  if (assetsRes.error && vendorResult.error) {
    return {
      assets: [],
      vendors: [],
      warnings,
      error: `${assetsRes.error.message}; ${vendorResult.error}`,
    };
  }
  ...
  const vendors = vendorResult.data as MaintenanceVendorOption[];
