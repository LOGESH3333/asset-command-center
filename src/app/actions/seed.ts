...
export async function seedDemoDataAction(): Promise<{ success?: boolean; error?: string; summary?: string }> {
  try {
    ...
    const assetsForInsert = assetsPayload.map((asset) =>
    ...
    const allocationRows = (assets ?? [])
      .map((a) => {
        const source = assetsPayload.find((asset) => asset.asset_tag === a.asset_tag);
        return {
          ...a,
          assigned_employee_id: source?.assigned_employee_id ?? null,
        };
      })
    ...
    if (allocationRows.length) {
      for (const allocation of allocationRows) {
        const result = await createAssetAllocationRow(allocation);
        if (result.error) throw new Error(`Allocations: ${result.error}`);
      }
    }
