...
function NewAllocationForm() {
  ...
  useEffect(() => {
    getAllocationFormLookupsAction(requestId).then((r) => {
      setAssets(r.assets ?? []);
      setUsers(r.users ?? []);
      setLinkedRequest(r.request ?? null);
      if (r.assets?.length === 1) {
        setAssetId(r.assets[0].id);
      }
      if (r.request?.requester_id) {
        setUserId(r.request.requester_id);
      }
  ...
  if (assets.length === 0) {
    const registerHref = requestId
      ? `/dashboard/assets/new?requestId=${encodeURIComponent(requestId)}`
    ...
    return (
      <EmptyState
        ...
        description={
          linkedRequest
            ? `Register an asset linked to "${formatRequestLabel(linkedRequest.justification)}" before allocating it to the requester.`
            : 'All assets are currently allocated or under maintenance. Return an asset or register a new one before creating an allocation.'
        }
        action={
    ...
    <div className="mx-auto max-w-lg space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {linkedRequest
              ? `Fulfill request: ${formatRequestLabel(linkedRequest.justification)} (${linkedRequest.status})`
          ...
          <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {linkedRequest
                  ? 'Showing assets registered for this request.'
