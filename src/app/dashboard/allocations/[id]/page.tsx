...
import { canManageAllocations, isSuperAdmin } from '@/lib/auth/roles';
...
export default function AllocationDetailPage() {
  ...
  const isAllocatee = profile?.id === row.user_id;
  const canAcknowledge =
    !row.acknowledged_at &&
    (isAllocatee || (role && (isSuperAdmin(role) || canManageAllocations(role))));
  const canView = role !== 'Employee' || isAllocatee;

  if (!canView) {
    return <ErrorAlert message="You do not have permission to view this allocation." />;
  }
