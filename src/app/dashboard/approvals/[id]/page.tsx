...
import { decideApprovalAction, getApprovalAction } from '@/app/actions/brd/approvals';
...
import { canActOnApproval, validateApprovalStageForRequest } from '@/lib/brd/request-workflow';
...
export default function ApprovalDetailPage() {
  ...
  useEffect(() => {
    getApprovalAction(id).then((result) => {
  ...
  const requestStatus = row.asset_requests?.status ?? '';
  const stageMismatch = row.status === 'Pending'
    ? validateApprovalStageForRequest(row.approval_stage, requestStatus)
    : null;
  const canDecide =
    row.status === 'Pending' &&
    canManageApprovals(role) &&
    canActOnApproval(row.approval_stage, requestStatus);
        ...
        <CardContent className="space-y-4 text-sm">
          ...
          {stageMismatch ? (
            <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
              This approval is no longer actionable. {stageMismatch}
            </div>
          ) : null}
          {canDecide ? (
