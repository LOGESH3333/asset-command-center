...
import {
  listDisposalMetricsAction,
  listDisposalsAction,
} from '@/app/actions/brd/disposals';
...
export default function DisposalsPage() {
  ...
  useEffect(() => {
    listDisposalMetricsAction().then((result) => {
      console.log('[disposal-ui] metrics raw result', result);
  ...
  const fetchRows = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await listDisposalsAction({ status, page, pageSize: PAGE_SIZE });
      console.log('[disposal-ui] queue raw result', result);
      if (result.error) setError(result.error);
      else {
        setRows(result.data);
