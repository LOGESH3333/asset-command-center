...
import { listPurchaseOrderMetricsAction, listPurchaseOrdersAction } from '@/app/actions/brd/purchase-orders';
...
export default function PurchaseOrdersPage() {
  ...
  useEffect(() => {
    listPurchaseOrderMetricsAction().then((result) => {
      if (result.error) setError(result.error);
      else setAllRows(result.data);
  ...
  const fetchRows = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await listPurchaseOrdersAction({ status, search, page, pageSize: PAGE_SIZE });
      if (result.error) setError(result.error);
          ...
          toolbar={
            <div className="flex flex-col gap-3 md:flex-row md:items-center">
              ...
              <Select value={status} onValueChange={(v) => { setStatus(v ?? 'ALL'); setPage(1); }}>
                <SelectTrigger className="erp-dark-glass-interactive w-full rounded-xl border text-zinc-100 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] md:w-[180px]"><SelectValue placeholder="Status" /></SelectTrigger>
                <SelectContent>
