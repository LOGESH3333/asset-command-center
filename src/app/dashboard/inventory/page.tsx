...
import {
  ...
  listInventoryMetricsAction,
} from '@/app/actions/brd/inventory';
...
export default function InventoryPage() {
  ...
  useEffect(() => {
    listInventoryMetricsAction().then((result) => {
      if (result.error) setError(result.error);
      const items = result.data ?? [];
      if (process.env.NODE_ENV === 'development') {
        console.log('[inventory-ui] metrics loaded', { count: items.length });
      }
      setAllItems(items);
  ...
  const fetchRows = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await listInventoryItemsAction({ search, page, pageSize: PAGE_SIZE });
      if (result.error) setError(result.error);
      else {
        if (process.env.NODE_ENV === 'development') {
          console.log('[inventory-ui] table loaded', {
            total: result.total,
            pageRowCount: result.data.length,
            search,
          });
        }
        setRows(result.data);
